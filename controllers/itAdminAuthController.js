...
exports.register = async (req, res) => {
  try {
    ...
    const existsAny = await existsInModel(ITAdmin, { email, mobile });
    if (existsAny) return res.status(400).json({ message: "You’re already registered." });

    const hashedPassword = await bcrypt.hash(password, 10);
    const doc = await ITAdmin.create({ name, email, mobile, password: hashedPassword });
    ...
    (async () => {
      try {
        ...
        if (to.length) {
          ...
          const subject = `New it admin registration awaiting approval`;
          const html = `
            <p>Hello Admin,</p>
            <p>A new <strong>IT admin</strong> has registered and is awaiting approval.</p>
            <ul>
              <li>Name: ${doc.name}</li>
...
exports.login = async (req, res) => {
  try {
    ...
    const user = await ITAdmin.findOne({ email })
      .lean()
    ...
    const token = jwt.sign({ id: user._id, role: "it_admin" }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const resetUrl = `${frontendUrl}/set-new-password?token=${token}&role=it_admin`;
...
exports.resetPasswordWithToken = async (req, res) => {
  try {
    ...
    const user = await ITAdmin.findOne({ passwordResetToken: token, passwordResetExpires: { $gt: new Date() } });
